	...
	<div class="col-12">
		...
		<div class="card">
			<div class="card-body">
				...
				<form method="post" class="btn-submit" action="{{ route('add_expense.update',$data->id) }}">
					@csrf
                    @method('PUT')
					<div class="row myinput">
                        ...
                        <div class="form-group col-md-6 mb-2">
							...
							<div class="input-group">
								<select class="form-control" name="expense_id" id="expense_id" required>
									...
									<option @if($i->id == $data->expense_id) selected @endif value="{{$i->id}}">{{$i->title_en}}</option>
                                    @endforeach
                                    @endif
								</select>
							</div>
						...
						<div class="form-group col-md-6 mb-2">
							...
							<div class="input-group">
								...
								<input class="form-control" type="text" name="amount" id="amount"  required="" placeholder="@lang('add_expense.amount')" value="{{$data->amount}}">
							</div>
						</div>
